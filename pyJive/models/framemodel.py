...
class FrameModel(Model):
    ...
    def _get_matrix_lb(self, params, globdat):
        ...
        for elem in self._elems:
            ...
            sfuncs = self._shape.get_shape_functions()
            grads, weights = self._shape.get_shape_gradients([coords1d])
            elmatM = np.zeros((self._dofcount, self._dofcount))
            elmatG = np.zeros((self._dofcount, self._dofcount))
    ...
    def _get_WN_matrix(self, N, dN, omega, eps=0):
        WN = np.zeros((self._dofcount, self._dofcount))
        l0 = 1 / dN[1]
        c = np.cos(omega)
        s = np.sin(omega)
        n1 = N[0]
    ...
    def _get_WV_matrix(self, N, dN, omega, gamma=0):
        WV = np.zeros((self._dofcount, self._dofcount))
        l0 = 1 / dN[1]
        c = np.cos(omega)
        s = np.sin(omega)
        n1 = N[0]
