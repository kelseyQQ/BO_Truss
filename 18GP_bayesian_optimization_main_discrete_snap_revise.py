...
def compute_constraint_1(elements_dict):
    constr_value = []
    for elem_id, elem in elements_dict.items():
        constr = max(elem['sigma_buc'] - elem['sigma'], -20 - elem['sigma'], elem['sigma'] - 20)
        constr_value.append(constr)
...
def bayesian_optimization(n_iter=100, improvement_threshold=1e-3, patience=50, data_path="initial_data_10000.npz"):  
    ...
    valid_idx = np.all(constr_samples_unscaled <= np.array([0] * 18)[:, np.newaxis], axis=0)
    ...
    kernel_constr = []
    for i in range(18):
        kernel_constr.append(Matern(length_scale=[1.0] * 12, length_scale_bounds=(0.1, 100), nu=1.5))
    ...
    for iteration in range(n_iter):
        ...
        if iteration % 5 == 0:
            ...
            optimized_kernel_constr = []
            for i in range(18):
                optimized_kernel_constr.append(gp_constr[i].kernel_)
        ...
        # ax[0].bar(range(1, 19), np.array(prob_plot_array[-1]).reshape(18))
        # ax[1].bar(range(1, 19), constr_next_unscaled)
        # plt.show()
    ...
    plt.legend()
    plt.savefig("best_weight_vs_iteration.png", dpi=300, bbox_inches="tight")
    plt.close()
...
if __name__ == "__main__":
    for i in range(1):
        ...
        variables, weight = bayesian_optimization(data_path="initial_data.npz")
        end_time = time.time()
